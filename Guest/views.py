...
def post(request):
    if request.method == "GET":
        form = ApartmentForm()  # Create an empty form instance
        context = {
    ...
    try:
        ...
        print(f"Error processing apartment listing: {str(e)}")
        return JsonResponse({
            'success': False,
...
#             messages.success(request, 'Hirdetés létrehozva!')
...
#     except Exception as e:
#         # Log the error for debugging
#         print(f"Error processing house listing: {str(e)}")
...
def posth(request):
    if request.method == "GET":
        form = HouseForm()  # Create an empty form instance
        context = {
            'form': form,  # Add form to context
            'user': request.user
        }
